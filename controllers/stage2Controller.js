...
exports.postStage2 = async (req, res) => {
  ...
  const aiWeights = await stage2Questions
    .findOne({
  ...
  const swWeights = await stage2Questions
    .findOne({
      "questions.questionNumber": questionNumber,
    })
  ...
  try {
    ...
    if (existingQuestion) {
      ...
      if (questionNumber <= lastQuestionNumber) {
        ...
        questionToUpdate.aiWeight += aiWeightsArray[Number(answerChar) - 1][0];

        questionToUpdate.swWeight += swWeightsArray[Number(answerChar) - 1][0];
        ...
        questions: [
          {
            userEmail,
            questionNumber,
            aiWeight: aiWeightsArray[Number(answerChar) - 1][0],
